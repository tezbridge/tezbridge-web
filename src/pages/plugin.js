...
function openSigner() {
  const signer = window.open('', 'TezBridge')

  try {
    if (signer.location.href === 'about:blank')
      signer.location.href = `${host}/index.html?signer`

    return {signer, is_prev: false}
  } catch (e) {
    return {signer, is_prev: true}
  }
}
...
class TezBridge {
  ...
  ready() {
    if (this.signer && !this.signer.closed) {
      ...
      const {signer, is_prev} = openSigner()
      this.signer = signer
